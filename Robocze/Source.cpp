...
int main() {
	...
	ALLEGRO_BITMAP *head = al_load_bitmap("glava.png"); // lepek weza
	ALLEGRO_BITMAP *body = al_load_bitmap("telo.png"); // reszta weza

	ALLEGRO_FONT *font1 = al_load_font("Pixel.ttf", 15, 0);
	ALLEGRO_FONT *mcfont = al_load_font("Pixel.ttf", 30, 0);
	ALLEGRO_BITMAP *coin1 = al_load_bitmap("coin.png");


	int count = 5;
	ALLEGRO_TIMER *timer = al_create_timer(1.0 / count);
		...
		if (events.type == ALLEGRO_EVENT_TIMER) {
			...
			if (events.timer.source == timer) {
				...
				if (x == coinX && y == coinY) {
					score++;
					count++;
					al_set_timer_speed(timer, 1.0/count);
				...
				if (menu == false) {
					for (int i = 0; i < score; i++) {
						if (x == snakeX[i] && y == snakeY[i] && menu == false)
						{
							count = 5;
							al_set_timer_speed(timer, 1.0 / count);
							dead = true;
					...
					if (x < 0 || x >= 800 || y < 0 || y >= 600 && menu == false)
					{
						count = 5;
						al_set_timer_speed(timer, 1.0 / count);
						dead = true;
